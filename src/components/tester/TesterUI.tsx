...
import { HardDrive, Laptop, Smartphone, Monitor, ChevronDown, ChevronUp, Loader2 } from "lucide-react";
...
import type { SessionSettings } from "@/lib/testerTypes";

// --- Visual Helpers ---

export const getCategoryIcon = (category?: string) => {
  const c = (category ?? "").toString().trim().toLowerCase();
  if (!c) return <HardDrive className="h-4 w-4" />;
  if (/\b(macbook|surface|laptop)\b/.test(c)) return <Laptop className="h-4 w-4" />;
  if (/\b(phone|iphone|android)\b/.test(c)) return <Smartphone className="h-4 w-4" />;
  if (/\b(chromebox|monitor|display)\b/.test(c)) return <Monitor className="h-4 w-4" />;
  return <HardDrive className="h-4 w-4" />;
};

export function printBarcode(svgElement: SVGSVGElement | null) {
  if (!svgElement) { alert("Barcode element not found. Cannot print."); return; }
  const svgData = new XMLSerializer().serializeToString(svgElement);
  const dataUrl = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(svgData);
  const printWindow = window.open("", "_blank", "width=600,height=500");
  if (!printWindow) { alert("Popup blocked. Please allow popups for this site."); return; }
  printWindow.document.write(`<html><head><title>Barcode</title><style>@page{size:auto;margin:0}html,body{margin:0;padding:0;height:99vh;overflow:hidden;display:flex;align-items:center;justify-content:center}.label{width:3.43in;height:0.6in;display:flex;align-items:center;justify-content:center;padding-top:1.0in;padding-right:0.25in}img{display:block;max-width:3.2in;max-height:1in;object-fit:contain}</style></head><body onload="window.print();window.onafterprint=()=>window.close()"><div class="label"><img src="${dataUrl}" /></div></body></html>`);
...
export function TinySpinner({ size = 16 }: { size?: number }) {
  return <span aria-hidden className="inline-block animate-spin rounded-full border-2 border-black/20 border-t-transparent" style={{ width: size, height: size, animationDuration: "800ms" }} />;
}
